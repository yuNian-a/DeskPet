...
def _send(state: int, msg: str = "") -> None:
    ...
    try:
        with _SEND_LOCK:
            ...
            sock.close()
            print(f"[hook] Sent state={state}", file=sys.stderr)
...
def _route_tool(data: dict) -> None:
    ...
    # Check both lowercase and original case variants
    base_lower = base.lower()
    exec_tools_lower = {t.lower() for t in _EXEC_TOOLS}
    proc_tools_lower = {t.lower() for t in _PROC_TOOLS}
    interactive_tools_lower = {t.lower() for t in _INTERACTIVE_TOOLS}

    if base_lower in interactive_tools_lower or raw.startswith("ask"):
        _send(7, "Awaiting your response...")
    elif base_lower in exec_tools_lower or raw.startswith("mcp"):
        _send(3, label)
    elif base_lower in proc_tools_lower:
        _send(2, label)
    else:
        _send(3, label)
